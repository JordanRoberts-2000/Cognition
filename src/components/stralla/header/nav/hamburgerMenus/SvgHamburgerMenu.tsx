...
const SvgHamburgerMenu = (({
    // introAnimationSpeed && outro ///////////////////////////////////////////////////////////////////////////////////////////////
    className, thickness, size, animationType, background, gap, hideBehindNav, stayInPlace,
    rounded, MenuToCloseSpeed, CloseToMenuSpeed}: Props) => {
    const hamburgerRef = useRef<HTMLButtonElement>(null)
    const headerContext = useContext(HeaderContext);
    let active = headerContext!.active
    const handleClick = () => {
        headerContext!.setActive!((prev) => !prev)
    }
    useEffect(() => {
        if(active){
            // if(stayInPlace && animationType){
            //     requestAnimationFrame(() => {
            //         hamburgerRef.current!.style.transform = `translate(-${headerContext!.navRef.current!.offsetWidth}px, 0)`
            //     })
            // }
            // if(stayInPlace && stayInPlace === 'right'){
            //     requestAnimationFrame(() => {
            //         hamburgerRef.current!.style.transform = `translate(${headerContext!.navRef.current!.offsetWidth}px, 0)`
            //     })
            // }
    ...
    return (
        <button style={cssVariables} ref={hamburgerRef} onClick={() => handleClick()}
            className={twMerge(buttonVariants({active, animationType, background, className}))}>
                <div className={twMerge(svgContainerVarient({active, rounded}))}>
                    {(animationType === 'svgSwirl' || animationType === undefined) &&
                        <svg viewBox="0 0 100 100" className={`${active && 'rotate-45'} transition-all w-full h-full scale-150 duration-500`}>
                            <path className='transition-all duration-500' fill='none' strokeWidth={'var(--hamburger-thickness)'} stroke={'black'}
                            ...
                            <path className='transition-all duration-500' fill='none' strokeWidth={'var(--hamburger-thickness)'} stroke={'black'} d="m 70,50 h -40" />
                            <path className='transition-all duration-500' fill='none' strokeWidth={'var(--hamburger-thickness)'} stroke={'black'}
