...
const shiftingAnimationFunctionMap : { [key: string] : (ref: HTMLElement, navRef: HTMLElement) => void} = {
...
const ShiftingMainContent2 = ({children, variant, className, animationVariant}: Props) => {
    const slidingRef = useRef<HTMLDivElement>(null)
    const { active, navRef } = useContext(HeaderContext);
    useEffect(() => {
        if(active){
            if(animationVariant && shiftingAnimationFunctionMap[animationVariant]){
                shiftingAnimationFunctionMap[animationVariant](slidingRef.current!, navRef!.current!)
            }
        }else{
            requestAnimationFrame(() => {
...
export default ShiftingMainContent2